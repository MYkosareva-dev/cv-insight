...
| Deploy | Vercel (Hobby) | All secrets in Vercel dashboard only. v2.24 preconditions, none of them enforceable from this repository: **Fluid compute ON** (without it the duration ceiling is 60 s and every chat route is over it — Block D, "Function duration on the deployment plan"), **function region `fra1`** (the default is `iad1`, which would process EU personal data in the US while /privacy says Frankfurt), and **registration closed at Supabase** (Password Protection is not a Hobby feature, so the app's own auth fence is the gate — see the v2.24 note and `docs/reviews/phase-6-vercel-security.md`) |
...
| `/privacy` | Privacy statement (static) | all |
| `/impressum` | Operator identity (static, v2.24) | all |
...
> 8. **`maxDuration = 120`** on the route: worst case is a 60 s chat attempt, the 2 s network-retry wait, a second 60 s attempt, then the embeddings and one RPC per requirement. A platform cap below that kills the request before `after()` runs and drops the `llm_calls` row for a call that WAS billed — rule B8 would stop holding with /quality as the only witness. **CHECKED AT THE PHASE-6 GATE, AND THE NUMBER STANDS (v2.24, closing backlog `p3-2` and `p4-1`).** See "Function duration on the deployment plan" below for the verified ceiling, the values every route declares, and what a run that exceeds one actually does.
...
### Function duration on the deployment plan (v2.24 — closes backlog `p3-2` and `p4-1`)

`p3-2` and `p4-1` both said the same thing: the app declares function-duration budgets and *nothing has ever checked them against the plan the app will actually run on*. Checked now, at the `vercel-security` gate. **The premise both items carried was stale** — 120 s does not exceed the Hobby ceiling, and neither does 300 s.

**The verified ceiling.** Vercel's own limits page (`/docs/functions/limitations`, last updated 2026-08-24) gives, for the Node.js runtime **with Fluid compute**, which is enabled by default for new projects:
...
**Without** Fluid compute the Hobby maximum is 60 s. So the ceiling this app lives under is not a property of the plan alone — it is a property of the plan *and one project toggle*, and that is the thing nobody had written down.

**What the routes declare, and all of it is inside 300 s:**

| Route | `maxDuration` | Why |
|---|---|---|
| `/api/applications/[id]/generate` | 300 | four chat steps; the route's own worst case is ~248 s |
| `/api/scan` | 120 | one chat attempt + the 2 s retry wait + a second attempt, then embeddings and one RPC per requirement |
| `/api/applications/[id]/judge` | 120 | one chat step with the same retry budget |
| `/api/dev/reindex`, `/api/dev/coverage-probe` | 120 | development only; unreachable on a deployment (Block H item 9) |
| `/api/applications/[id]/export`, `/api/applications/[id]/rescore` | 60 | no chat call on either path |

**DEPLOYMENT PRECONDITION: Fluid compute must be ON.** With it off the Hobby ceiling is 60 s and four of the seven routes are over it, including every route that makes a chat call. This is a dashboard setting (Settings → Functions), so nothing in this repository can enforce it or even observe it — which makes it exactly the class of thing this project writes down rather than assumes. Confirm it before the first deploy; the Max Duration field refusing a number is the authoritative answer for the account.

**What happens to a generate-with-judge run that needs longer than its budget.** Traced end to end rather than assumed, because "the user must see a real message, not a truncated response" is the requirement:

1. Vercel terminates the invocation and returns **504 `FUNCTION_INVOCATION_TIMEOUT`**. It does not return a partial or truncated body — the response is the platform's error, not a half-written resume.
2. The client's `run()` in `src/components/applications/result-workspace.tsx` does `await res.json().catch(() => null)` on any non-OK response. A 504 page is not JSON, so the parse fails, the result is `null`, and the toast falls back to the action's own message.
3. For generate that message is `RESULT.generationFailed` — *"Generation failed — nothing was saved. Try again."* — **and it is TRUE, not merely present.** Both `resume_versions` writes happen after the whole pipeline returns; a run killed mid-pipeline has written no version. The user is told something accurate and is offered the retry, which is a button they press (CLAUDE.md: a retry is not something the app does on its own).
4. **The real cost of an overrun is the audit row, not the user's experience.** `logLlmCall` is scheduled through `after()`, which does not run when the platform kills the invocation, so a run that spent money before dying leaves no `llm_calls` row. Rule B8 stops holding for that request and `/quality` — the app's only witness to its own spending — is silently short by one row. That is the reason to keep the ceiling comfortably above the worst case, and the reason this table exists at all.
...
**`/login`, `/signup`** — centered card 400 px; fields Email, Password; primary green button [Sign in]/[Create account]; link to the other form; footer links Privacy and Impressum.

> **v2.25 — ON A DEPLOYMENT, `/signup` HAS NO FORM (gate finding `vs-1`).**
>
> The deployment was to be gated by Vercel Password Protection. That is a Pro feature and this project is on Hobby, so the gate moved: the URL is reachable, and what keeps strangers out is **new sign-ups disabled in the Supabase dashboard**, with accounts created by hand for named people. With that setting on, `signUpAction` cannot succeed — Supabase refuses it. Rendering the form anyway would offer a control whose only possible outcome is a refusal.
>
> `/signup` therefore keeps its route and explains. Copy, verbatim:
>
...
> >
> > If an account was created for you, sign in instead.
> >
> > [Sign in] *(link to `/login`)*
>
> **`/login` drops its "No account? Create one" link** on a deployment as well. A link whose destination exists only to say no is worse than no link. `AuthForm`'s `altHref`/`altLabel` become optional together, so omitting them renders nothing and passing one without the other is a type error.
>
> **The discriminator is `NODE_ENV`, not a new environment variable.** It is what this codebase already uses for "this is a deployment" — the same line as the `/api/dev/*` fence, which is witnessed against a production build in Block H item 9. A dedicated `REGISTRATION_OPEN` flag would be a second source of truth about one fact, settable to the wrong value, and it still would not be the gate. The gate is the Supabase setting; this is the page telling the truth about it.
>
> **Consequence for the Playwright suite, recorded here because it is easy to discover the hard way.** All four e2e specs create a throwaway account through this form. They run against a development server, so the form is still there for them — but they talk to the SAME Supabase project, so once sign-ups are disabled there the suite cannot create accounts either, whatever this page renders. `docs/deploy.md` step 2 covers the ordering that follows from this.

**The demonstration notice on the authenticated shell** (v2.25, gate finding `eu-2`). Rendered in the `(app)` layout, above the page content, on every member screen. Copy, verbatim:

> **Demonstration deployment.** No other person’s real resume or personal details may be uploaded to it — use your own career history, or invented sample data.

On the SHELL and not on `/scan`, because the constraint is not about one screen: text arrives through the scan panel, the import dialog and the editor, and a notice attached to one of them is absent from the other two. It has **no dismiss control**, deliberately — a notice the user can close is a notice that is absent for every session after the first, and this one has to hold for the life of the deployment. Styled as a quiet standing condition, not an alarm demanding action. It is removed when the operator has their own model-provider account and the verification is recorded in `docs/openrouter-processing.md`; the same event retires the matching paragraph on `/privacy`.
...
> **v2.24 — the completeness pass, as built.** The page above was ACCURATE and badly INCOMPLETE, and the two are not the same property: every sentence on it was true, and most of Art. 13 was simply absent. Added: the controller (by link to `/impressum`), **a legal basis per purpose** (Art. 6(1)(b) for the service, Art. 6(1)(f) for the authentication audit records, and the affirmative statement that nothing here runs on consent), the data categories the old list omitted (application notes and status, and the fact that `llm_calls` is metadata with no column content could occupy), **retention for everything that is not the audit records**, all six data-subject rights plus **the right to complain to a supervisory authority** — a flat omission before, and one that is never scaled by audience size — and a line saying no decision is made about anyone.
>
> **THE SERIOUS FIX IS THE TRANSFER.** The page said "EU (Frankfurt)" twice and never said that every model call leaves for the United States, so a careful reader would have concluded the opposite of what happens. It now names the transfer, the four model slugs, and the three companies behind them — OpenAI, Anthropic and Google — and names **Vercel**, a processor that terminates every request and was invisible on the page.
>
> **Two sentences the app would rather not print, and both are true.** (1) The contact-details guarantee is exact and compiler-enforced, and it covers the fields the APP adds — not text the USER imports. A resume PDF carries its own name, email and phone in its header, and importing one sends the document as it stands. Stating only the first half created the impression that no contact detail ever reaches the provider. (2) The OpenRouter account this deployment's key belongs to is not the operator's, so its logging, retention and training settings cannot be stated to a reader — and a reader cannot evaluate a transfer whose terms nobody will name. The page says so, and asks that a real resume not be pasted until it can say otherwise. Both sentences retire the day the operator has their own provider account.
>
> **The audit-retention ternary is untouched.** The retention PERIOD still appears in exactly one place — `PRIVACY_ERASURE`, switched by `AUDIT_RETENTION_VERIFIED`, gated by R12. The new erasure paragraph beside it names no period, which is why it can sit outside the switch: it is about the GRANULARITY of erasure, which is the honest admission that only career items are individually deletable and everything else goes with the account.

**`/impressum`** (v2.24) — static, public, reachable from BOTH layouts beside the Privacy link, and excluded from the middleware matcher as an EXACT path (`impressum$`, never a subtree) exactly as `/privacy` is. It is public in both deployment worlds: a page whose purpose is to identify the operator to someone who has not signed in cannot sit behind the sign-in.

Copy, verbatim. `IMPRESSUM_FILLED` in `src/lib/copy.ts` is the switch, and it ships **false**:

> **Impressum**
>
> Angaben gemäß § 5 DDG
>
> *(while `IMPRESSUM_FILLED` is false — the only text that renders)*
...
>
> *(once `IMPRESSUM_FILLED` is true)*
> `[[OPERATOR FULL LEGAL NAME — owner to fill]]`
> Contact: `[[OPERATOR CONTACT EMAIL — owner to fill]]`
> Responsible for the content: `[[OPERATOR FULL LEGAL NAME — owner to fill]]`
...
>
> How CV Insight handles personal data is described on the privacy page.

**The two values are the OWNER'S to fill and the agent does not invent them** — not from `git config`, not from a commit author, not from anywhere. An Impressum naming the wrong person is worse than one that admits it is unfilled, which is why there is a switch rather than a page that renders `[[...]]` at a reader as though that were an identity.

**Why no postal address, and when that stops being enough.** Owner decision, and defensible while this is a closed demo: §5 DDG binds *geschäftsmäßige* telemedia — a sustained, outward-facing offering — and a link shared with a named handful of people, not monetised and with no pricing anywhere in the product, is not one. Art. 13(1)(a) GDPR applies regardless and requires the controller be identifiable and contactable; a name plus a working email address plus an undertaking to supply the address on request satisfies that for this audience. **The day the deployment becomes public or commercial, name + email is no longer sufficient** — a public German site needs a *ladungsfähige Anschrift*, an address at which the operator can be served, and "on request" does not substitute for one. That condition is recorded beside the constant in `src/lib/copy.ts` as well as here, because `copy.ts` is the file someone actually edits.
...
#### Response security headers (v2.25 — gate findings `ns-2` / `vs-5`)

Set in `next.config.ts` via `headers()` on `source: '/:path*'`, so they cover static pages, member pages and route handlers alike — verified on all three, including a 401 from `/api/scan` and a 307 from `/scan`.

| Header | Value |
|---|---|
| `Content-Security-Policy` | `default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:; font-src 'self' data:; connect-src 'self'; form-action 'self'; frame-ancestors 'none'; frame-src 'none'; object-src 'none'; base-uri 'self'; worker-src 'self' blob:; manifest-src 'self'; upgrade-insecure-requests` |
| `X-Frame-Options` | `DENY` |
| `X-Content-Type-Options` | `nosniff` |
| `Referrer-Policy` | `strict-origin-when-cross-origin` |
| `Permissions-Policy` | `camera=(), microphone=(), geolocation=(), payment=(), usb=(), interest-cohort=()` |

**The policy is built from an inventory, not a template.** There is no CDN, no analytics package, no font host and no `public/` directory; Tailwind and `tw-animate-css` are bundled at build. The browser never contacts Supabase — `createBrowserClient` is banned and R11 enforces it — and every client fetch is a relative `/api/*` path, which is what lets `connect-src` be `'self'` rather than a Supabase origin.

**`script-src 'unsafe-inline'` is a MEASURED cost, not an assumed one.** The strict policy was built and served first, and it broke all five public pages: Next.js App Router streams the RSC payload through inline `<script>` tags, so blocking them fails hydration (React error #412) and leaves pages that look right and do nothing. `docs/eval/csp-verification.md` carries both transcripts and the positive hydration proof. **A nonce was rejected for a specific reason**: it must be minted per request in middleware, and `/privacy` and `/impressum` are deliberately outside the matcher so they stay static — a nonce would either miss exactly those two or drag them into the matcher and make them dynamic.

What the policy still buys: `frame-ancestors 'none'` closes the clickjacking exposure `ns-2` actually named — the account-deletion dialog on `/settings` — and no external origin is permitted for any resource type, so an injected `<script src>` pointing off-origin fails regardless of `'unsafe-inline'`, which permits only inline code. HSTS is deliberately absent: Vercel serves it on its own domains, and setting a `max-age` here would bind a future custom domain before anyone had decided that.

#### Function region (v2.25 — gate findings `vs-6` / `eu-10`)

`vercel.json` sets `"regions": ["fra1"]`. The platform default is `iad1` (Washington DC), so without this every request — including all resume and vacancy text — would be PROCESSED in the United States while `/privacy` named Frankfurt twice. Hobby permits a single region and this is one; deploying to more than the plan allows fails the deployment before the build.

It is stated in the repository rather than left to the dashboard so the setting is reviewable in a diff. **The limit of the claim, stated because `/privacy` depends on it:** Vercel deploys Routing Middleware across its network regardless of this setting, so `src/middleware.ts` is not pinned by it. Resume and vacancy text are only ever handled in route handlers and Server Components, which ARE the pinned functions; what the routing layer sees is the request and its session cookie, which is what `/privacy`'s IP-address disclosure already covers. The page therefore says the application and the database run in Frankfurt, and does not claim that literally everything does.
...
> **v2.25 — `eu-9` IS CLOSED, by owner-run evidence: `docs/eval/erasure-evidence.md`.**
>
> The gate asked whether the erasure promise on `/privacy` has anything behind it, since CLAUDE.md's Privacy rule says "verified by test". It now does, and the record is a measurement rather than an argument.
>
> **What was run**, on 2026-09-04 against the live project: a throwaway account was created while registration was still open, populated until **all eight owner-scoped tables held rows**, then deleted **through the app's own [Delete account and data] control** — not with a SQL `DELETE` — and the same per-user counts were taken in the Supabase SQL editor before and after. All eight go to zero (`career_items` 8→0, `documents` 24→0, `vacancies` 1→0, `applications` 1→0, `resume_versions` 2→0, `llm_calls` 9→0, `imports` 1→0, `profiles` 1→0) and the `auth.users` row is gone. Deleting through the product is the load-bearing detail: it exercises `DELETE /api/account`, the service-role client and the hard delete, where a statement issued beside the app would have proved Postgres's behaviour and nothing about the path a user takes.
>
> **It closes the way the finding recommended, which is why R10 is untouched.** The obvious test — count rows from Playwright after deleting — cannot be written: after deletion there is no session, so RLS makes "the rows are gone" indistinguishable from "the caller may not see them", and reading them needs the service-role key that `scripts/check.mjs` R10 pins to `src/lib/supabase/admin.ts`, with `tests/` inside its scope. The two options were a SQL-level check recorded in `docs/eval/`, or an R10 carve-out letting a test hold the key that bypasses RLS entirely. **The first was the recommendation and the first is what happened**, so the one key with no fence underneath it is still read in exactly one module.
>
> **`privacy.spec.ts` remains unwritten**, and this item no longer waits on it. Its delete-account half is covered by the evidence file plus the account-deletion case in `tests/e2e/auth.spec.ts`, which exercises the real dialog and proves the credentials stop working. Its other half is covered ALMOST but not exactly: `tests/e2e/generate.spec.ts` asserts 404-never-403 on every Phase-4 endpoint for an id that resolves to no row for the caller — but the id is a fabricated UUID, so it does not distinguish "no such application" from "someone else's application". Both answers travel the same RLS path and the same handler branch, so the gap is small; it is named here rather than rounded off, because a test that reads as cross-account coverage without quite being it is the kind of thing a later reviewer should not have to rediscover.
>
> **The limits are recorded in the evidence file rather than implied here**: it witnesses nothing about data held at OpenRouter, and nothing about `auth.audit_log_entries`, which survives deletion by design and has its own 90-day purge with its own succeeded-run record.
...
9. **The `/api/dev/*` routes are unreachable on the deployment, verified against a PRODUCTION BUILD** (v2.15, from backlog p3-22). Both instruments — `coverage-probe` and `reindex` — refuse with 404 when `NODE_ENV === 'production'`, before auth and before any argument is parsed, and both ship in the bundle. That fence cannot be unit-tested (the handlers import `server-only`) and the Playwright suite only ever runs against a development server, so nothing in the repo witnesses it: a third dev route that omitted the guard would pass `check`, every test and every gate. Owner-run, once, before the first deploy: `npm run build && npm run start`, then `curl -i localhost:3000/api/dev/coverage-probe?applicationId=<uuid>` and `curl -i -X POST localhost:3000/api/dev/reindex` — both must answer **404**, signed in or not. Paste both responses into `docs/eval/dev-routes-production-evidence.md`, the same shape as the auth audit-retention evidence beside it: the claim and its proof ship together, or the claim does not ship.
   **DONE — v2.24, 2026-09-04, commit `519aa4c`.** Built, started under `NODE_ENV=production`, both routes requested; both answer **404** with the app's own `NOT_FOUND` JSON, signed out and with cookies present. The run carries two controls, because a bare 404 proves less than it looks: a live metered route answers **401** to the same signed-out caller (so the 404 is not the auth check), and a genuinely absent route answers Next.js **HTML** (so the JSON 404 means the handler ran and the fence threw). The directory was globbed before the run rather than trusting "both" — exactly two route files, both carrying the guard. What it does not prove is recorded in the file: a THIRD dev route added later is still guarded by review alone, since the thirteen check rules are frozen and none covers it (`vs-10`).
