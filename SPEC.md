...
> **v2.25 — `eu-9`, ANSWERED RATHER THAN FIXED. The erasure promise is PARTLY witnessed, and `privacy.spec.ts` does not exist.**
>
> The compliance gate asked whether the erasure claim on `/privacy` has a test behind it, since CLAUDE.md's Privacy rule says "verified by test". Checked, and the answer is precise:
>
> **What IS witnessed today**, by `tests/e2e/auth.spec.ts` ("account deletion removes the user and returns to /login with the toast"): the deletion runs through the real dialog with its confirmation, the dialog's carve-out wording and its `/privacy` link are asserted, a `profiles` row is deliberately created first so the cascade covers eight tables rather than seven, and **the account is genuinely gone** — the same credentials afterwards produce the bad-credentials copy rather than a session. That is a real test of the auth user's deletion.
>
> **What is NOT witnessed**: that **0 owned rows** remain. Nothing counts rows in the eight tables after the delete. The cascade is `ON DELETE CASCADE` on all eight foreign keys and it is almost certainly correct, but "almost certainly correct by reading the schema" is the argument this project's own evidence rule exists to refuse.
>
> **Why it was not simply fixed here, which is the interesting part.** The obvious test — count rows for that user id after deleting — cannot be written from the browser: after deletion there is no session, so RLS returns nothing to anyone, and a query that returns nothing because the rows are gone is indistinguishable from one that returns nothing because the caller has no rights. Reading them requires the service-role key, and **`scripts/check.mjs` R10 fails the build on `SUPABASE_SERVICE_ROLE_KEY` read anywhere but `lib/supabase/admin.ts`** — its file predicate covers `tests/` as well as `src/`, verified. So the test would either not compile past `npm run check` or would require unfreezing the thirteen rules.
>
> **What it would take**, so the next person does not rediscover this: (a) a SQL-level fixture run against the database rather than through the app — insert a row in each of the eight tables for a throwaway user, delete the user, assert every count is zero — recorded in `docs/eval/` the way the audit-retention run is; or (b) an owner amendment carving `tests/` out of R10 so a Playwright spec may hold the service-role key, which widens the blast radius of the one key with no fence under it and is the worse trade. **(a) is the recommendation.** Until one of them is done, the `/privacy` erasure sentence rests on the schema plus a test that proves the account — not the rows — is gone.
