...
> 10. **`resumeContentSchema` bounds the editor at 100-15,000 characters**, which is `resume_versions.content`'s own CHECK, so an over-long or empty editor gets US-5's copy instead of a Postgres constraint error mapped to a 500. `generateResume` also truncates at that ceiling as a backstop to edge case L3: `max_tokens` makes an over-long resume unlikely, and "unlikely" is not what a CHECK constraint enforces - a violated one would turn a run the user has already paid for into "nothing was saved".
