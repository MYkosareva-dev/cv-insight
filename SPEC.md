...
> Version: 2.17 | Date: 2026-09-03 | Status: Production-ready
...
> 4. **THE MECHANISM IS THE TYPE SYSTEM.** `JudgeCard` no longer reads `keywordCoverage.missingHonest` from the report and takes the partition as a REQUIRED prop, so no render site can reach the raw list by forgetting to filter — the compiler caught the one that tried while this was being wired. The workspace holds the report and its partition as ONE piece of state for the same reason. The STORED report keeps the reviewer's own words: it is the record of what the review said, and what is bound is what the screen and the prompt may do with it.
> 5. **THE BASE NEVER CROSSES THE WIRE.** All three sites that render a review — the detail page, `/generate`, `/judge` — compute the split on the SERVER. A career base runs to hundreds of kilobytes of the user's own history and the browser needs one yes-or-no per term, not the corpus. The page checks against the WHOLE base rather than the retrieved items, because a stored report may have been written against a retrieval it cannot reproduce, and the broader corpus is the conservative direction — it can only admit a term the user really does have.
> 6. **P2 AND P3 BOTH RECEIVE `{{candidateName}}`** (Block F templates amended). P2 gains rule 6: the NAME line is the supplied name, copied character for character, never a job title or a company name. P3 is told the name comes from the user's profile and is NOT a claim to check — without that the grounding gate fires on the one line the user supplied themselves, and rule B2 makes that uncompensatable, so every generated resume would be revised once for having a name on it.
