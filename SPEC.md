...
> Version: 2.8 | Date: 2026-09-02 | Status: Production-ready
...
> **R12 fails CLOSED, and is deliberately over-inclusive.** It matches any period expression (number + day/days/week/month/year, hyphenated or not, digits or words) appearing near ANY of the retention vocabulary this app actually uses — `audit`, `authentication record`, `retention`, `log entr`. It does not encode one blessed sentence: the app's own shipped phrase was "Some authentication records", which a rule keyed on the word "audit" cannot see. A false positive is resolved by adding the evidence file or rephrasing the copy — NEVER by narrowing the rule. Its evidence predicate anchors on a line matching `^\s*status:\s*succeeded\b`, not a substring, so a file recording "has NOT succeeded" does not unlock the claim.
