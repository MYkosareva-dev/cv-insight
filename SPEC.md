...
> **Angle brackets are refused as well**, on `cleanDisplayName`'s reasoning: a stored URL is interpolated into the resume text P3 reads inside its `<resume>` block, so a value carrying `</resume>` would close that block early and put the rest of itself outside the region the prompt marks as data. No well-formed URL contains a literal `<` or `>`, so this costs nothing and removes the new column as a second author of that block. Every field's MINIMUM length is the column's own, for the same subset reason.
> Decision (an EXPORT with no header says so): `withContactHeader` runs at GENERATION time, so a resume written before the contact details were saved has none — and `/export` writes the editor's text verbatim, which is the point: the document on disk is the text the user saw and edited, not a second composition that could differ from it. That leaves one reachable state where the very defect 005 exists to fix would persist silently, so the export answers `X-Missing-Contacts: 1` when the profile HAS contacts and the document contains none of the lines the header would have carried, and the client raises `RESULT.exportedWithoutContacts`. Detected with the same function that composes the block, so the two cannot hold different opinions about what a header is. A WARNING and never a refusal, like the name placeholder beside it: the file is the user's.
...
> **/privacy names the TRANSFER as well as the storage** (`docs/reviews/phase-5-architect-diff.md`, finding 7). The header block is inserted before the judge step, so the email, the phone number, the location and both URLs go to OpenRouter inside P3's `<resume>` block on every generate and every [Check quality]. The first version of the paragraph said the details were used "for one thing: the header block", which is true of the PURPOSE and silent about where the data goes — and the "Processing by OpenRouter" section named only resume and vacancy text. Both now say it. **One open item for the owner**: CLAUDE.md's Privacy section names "Resume and vacancy text" as what is sent to OpenRouter. The contact block is part of that resume text, so the sentence is still true — but it is less specific than reality, and widening it is an owner amendment rather than something to take in a feature branch.
