...
>
> **What this does NOT do.** It does not touch the thresholds, the chunker, or keyword counting; it adds no model call and no migration. And it does not make coverage correct in general — it removes one specific, measured class of false positive, the one where the deciding evidence is a name. `docs/eval/coverage-thresholds.md` Part 3 carries the before/after status of every requirement in the seeded case, including the confirmation that no `general` requirement changed status.
